...
const regexMock = /<button[^>]*onClick=\{\(e\) => \{ e\.preventDefault\(\); alert\("Mocked Google Login"\); \}\}[^>]*>[\s\S]*?<\/button>/;
const replacementMock = `<div className="flex justify-center w-full mt-6"><GoogleLogin onSuccess={handleGoogleSuccess} onError={() => setError('Google login failed')} useOneTap /></div>`;
