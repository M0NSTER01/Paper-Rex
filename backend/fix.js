...
const injection = `
      ...
      chatForm.addEventListener('submit', async (e) => {
        ...
        try {
          ...
          if(resData.response) {
            chatMessages.innerHTML += \`<div class="self-start bg-gray-200 text-gray-800 p-3 rounded-2xl rounded-tl-none text-sm max-w-[85%]">\${resData.response.replace(/</g, "&lt;")}</div>\`;
...
if (!c.includes('CHATBOT WIDGET')) {
    c = c.replace(/      if \(contactForm\) \{/, injection);
